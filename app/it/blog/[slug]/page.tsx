...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: `https://potastudio.com/it/blog/${slug}`,
      languages: {
    ...
    openGraph: {
      ...
      title: post.title,
      description,
      url: `https://potastudio.com/it/blog/${slug}`,
...
export default async function BlogPostPageIT({ params }: Props) {
  ...
  const articleSchema = {
    ...
    author: {
      ...
      name: authorName,
      jobTitle: authorRole,
      url: 'https://potastudio.com/it/about',
    },
    publisher: {
      '@type': 'Organization',
      name: 'Pota Studio',
    ...
    mainEntityOfPage: { '@type': 'WebPage', '@id': `https://potastudio.com/it/blog/${slug}` },
    inLanguage: 'it',
    keywords: [post.primaryKeyword, ...(post.secondaryKeywords ?? [])].filter(Boolean).join(', '),
  }
