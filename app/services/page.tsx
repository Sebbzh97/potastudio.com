...
/** Read up to 5 deliverables for service N from the Sanity data bag. */
...
export default async function ServicesPage() {
  ...
  // Build services list from Sanity. If no title, skip.
  ...
  const ctaHref      = data?.ctaButtonHref  ?? '/contact'
  const serviceCtaLabel   = data?.serviceCtaLabel   ?? 'Start this service'
  const deliverablesLabel = data?.deliverablesLabel ?? "What's included"
    ...
    <main>
      ...
      <section className="pt-28 sm:pt-40 pb-16 sm:pb-24 bg-[#0D0D0D] relative overflow-hidden">
        ...
        <div className="container-site relative">
          ...
          {/* `{' '}` before <br /> preserves whitespace in the SSR textContent
              so crawlers / LLMs do NOT read "Services designedto scale." as
              one concatenated token. */}
          ...
          <div className="container-site relative z-10">
            <div className={`grid grid-cols-1 lg:grid-cols-2 gap-10 sm:gap-16 items-start ${i % 2 === 1 ? 'lg:grid-flow-dense' : ''}`}>
              <div className={i % 2 === 1 ? 'lg:col-start-2' : ''}>
                ...
                <Link href="/contact" className="inline-flex items-center gap-2 px-5 sm:px-6 py-3 bg-[#FF5C00] text-white font-semibold rounded transition-all duration-200 hover:bg-[#e04f00] hover:shadow-[0_0_20px_rgba(255,92,0,0.4)] group text-sm sm:text-base">
